...
pub fn run_live(args: &Args) -> Result<()> {
    ...
    let tracker_handle = thread::Builder::new()
        ...
        .spawn(move || {
            ...
            for frame in frame_rx {
                ...
                let now = unix_now_ms();
                match tracker.ingest(key, direction, seg.payload, now) {
                    Ok(Some(state)) => {
                        if tracker_ui_tx
    ...
    // Signal shutdown to helper threads by dropping the ui_tx clone we
    // still hold and letting the receivers terminate naturally. Then join
    // for cleanliness.
    drop(ui_tx);
